...
namespace InventorySystem
{
    ...
    /// <summary>
    /// 이 클래스는 인벤토리 컨트롤러를 정의합니다. 이 컨트롤러를 사용하여 새 인벤토리를 생성하고 유효한 객체 유형을 정의할 수 있습니다.
    /// 프로젝트 내에서는 하나의 InventoryController만 인스턴스화되어야 합니다. 하나의 컨트롤러로부터 여러 인벤토리를 생성할 수 있습니다.
    /// 이 컨트롤러는 인벤토리에 전달되는 모든 정보를 관리합니다.
    /// </summary>
    public class InventoryController : MonoBehaviour
    {
        [Space(10)] // 조직을 위해 일부 공간을 추가합니다.

        [Header("=====[ 인벤토리 컨트롤러 설정 ]=====")]
        [Space(20)]
        [Tooltip("기본 UI 캔버스를 할당합니다.")]
        [SerializeField]
        ...
        [Header("=======[ 아이템 항목 설정 ]=======")]
        [Header("참고: 항목에 대한 모든 변경 사항은 여기에서 수행되어야 합니다.")]
        [Tooltip("허용 가능한 각 인벤토리 항목에 대한 템플릿을 추가합니다.")]
        [SerializeField]
        public List<ItemInitializer> items; // 인벤토리에 추가할 수 있는 항목 목록.

        [Space(10)] // 공간을 추가합니다.

        [Header("========[ 인벤토리 설정 ]========")]
        [Header("참고: 초기화 후 여기서의 변경 사항은 적용되지 않습니다.")]
        [Header("UI 구성 요소 아래의 인벤토리를 수정합니다.")]
        [Tooltip("초기화할 각 인벤토리에 대한 템플릿을 추가합니다.")]
        [SerializeField]
        public List<InventoryInitializer> initializeInventory = new List<InventoryInitializer>(); // 인벤토리 설정에 관한 정보.

        [SerializeField, HideInInspector]
        private List<InventoryInitializer> prevInventoryTracker; // 이전에 초기화된 인벤토리 목록으로, 다시 초기화되지 않도록 합니다.

        [Tooltip("각 인벤토리 UI를 제어하는 인벤토리 매니저의 프리팹입니다.")]
        [SerializeField]
        private GameObject inventoryManagerObj; // 인벤토리 매니저를 위한 프리팹.
